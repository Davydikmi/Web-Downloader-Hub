...
{
    public class InstagramDownloadHandler:IDownloadHandler
    {
        public string Download(string url)
        {
            ...
            string fileNameTemplate = Path.Combine(downloadDir, "instagram_%(id)s.%(ext)s");
            ...
            if (process.ExitCode != 0)
            {
                ...
                throw new Exception($"Ошибка при загрузке с Instagram: {error}");
            }
            ...
            if (latestFile == null || !latestFile.Exists)
                throw new Exception("Медиафайл из Instagram не найден после загрузки.");
